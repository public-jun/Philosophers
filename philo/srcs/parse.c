...
int	check_valid_philo_value(t_info *info)
{
	if (is_in_range(info->num_philo_and_fork, PHILOMIN, PHILOMAX) == false)
		return (-1);
	if ((is_in_range(info->time_to_die, MSECMIN, MSECMAX) == false)
		|| (is_in_range(info->time_to_eat, MSECMIN, MSECMAX) == false)
		|| (is_in_range(info->time_to_sleep, MSECMIN, MSECMAX) == false))
		return (-1);
	if ((info->flag_must_eat == true)
		&& (is_in_range(info->num_times_must_eat,
...
int	parser(int ac, char **av, t_info *info)
{
	...
	if (set_philo_data(&(info->time_to_sleep), av[4]) == -1)
		return (-1);
	if (info->flag_must_eat == true
		&& set_philo_data(&(info->num_times_must_eat), av[5]) == -1)
		return (-1);
